...
from __future__ import print_function # function for printing (explained above)
from .exceptions import MpesaConfigurationException, IllegalPhoneNumberException, MpesaConnectionError, MpesaError # error handling
from main.models import AccessToken  # main is the app name and accessToken is our model
import requests # Make a request to a web page
from django.utils import timezone # for showing the current time
from decouple import config, UndefinedValueError # Separates sensitive info from others such as Secret Key, Middlewares, and Installed Apps
from requests import Response # For server's response to the HTTP request
import time #  to handle time-related tasks
import os
from django.conf import settings #abstracts the concepts of default settings and site-specific settings; it presents a single interface. It also decouples the code that uses settings from the location of your settings.
import base64 # Encrypt password using given RSA public key( PKCS1v15()) and encode it with base64
from cryptography import x509 # used for loading certificates :: --https://cryptography.io/en/latest/x509/reference/--
from cryptography.hazmat.primitives.asymmetric.padding import PKCS1v15 #  RSA public key
...
def mpesa_response(r):
    ...
    r.__class__ = MpesaResponse #   r.__class__ ==  MpesaResponse object from requests.Response object
...
def mpesa_access_token():
    ...
    # 	"""
	# Generate access token if the current one has b_token expired or if token is non-existent
